...
@st.cache_data
def load_and_clean_data(customers, orders, payments):
    try:
        # Gabungkan dataset
...
# Meminta user untuk mengunggah file CSV
st.sidebar.title("Unggah Dataset")
uploaded_customers = st.sidebar.file_uploader("Unggah olist_customers_dataset.csv", type="csv")
uploaded_orders = st.sidebar.file_uploader("Unggah olist_orders_dataset.csv", type="csv")
uploaded_payments = st.sidebar.file_uploader("Unggah olist_order_payments_dataset.csv", type="csv")

if uploaded_customers is not None and uploaded_orders is not None and uploaded_payments is not None:
    customers = pd.read_csv(uploaded_customers)
    orders = pd.read_csv(uploaded_orders)
    payments = pd.read_csv(uploaded_payments)
    
    # Memuat data
    data = load_and_clean_data(customers, orders, payments)
    ...
    st.error("Silakan unggah ketiga file dataset untuk melanjutkan analisis.")
